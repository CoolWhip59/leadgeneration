...
const places_service_1 = require("./modules/places/places.service");
const website_check_service_1 = require("./modules/website-check/website-check.service");
const prisma_service_1 = require("./modules/prisma/prisma.service");
const queue_constants_1 = require("./modules/queue/queue.constants");
async function bootstrap() {
    const app = await core_1.NestFactory.createApplicationContext(app_module_1.AppModule, {
        logger: ['error', 'warn', 'log'],
    });
    const prisma = app.get(prisma_service_1.PrismaService);
    const places = app.get(places_service_1.PlacesService);
    const websiteCheck = app.get(website_check_service_1.WebsiteCheckService);
    const connection = new ioredis_1.default(process.env.REDIS_URL || 'redis://localhost:6379', {
        maxRetriesPerRequest: null,
    });
    const worker = new bullmq_1.Worker(queue_constants_1.LEAD_QUEUE_NAME, async (job) => {
        ...
        for (const place of results) {
            ...
            const shouldCheck = !business.websiteCheckedAt ||
